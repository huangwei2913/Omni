...
class BunnyMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        ...
        # ==========================================================
        # ✨ 核心修复：检查 mm_projector 的第一个参数的设备
        # 通过 next(iter()) 获取第一个参数，然后检查它的设备
        try:
            projector_device = next(mm_projector.parameters()).device
        except StopIteration:
            # 如果 mm_projector 没有参数（不常见，但安全起见）
            projector_device = image_features.device 
            
        if projector_device != image_features.device:
            print(f"Warning: Forcing mm_projector from {projector_device} to {image_features.device} inside encode_images.")
            mm_projector = mm_projector.to(image_features.device)
        # ==========================================================
